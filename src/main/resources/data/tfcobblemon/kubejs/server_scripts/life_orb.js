...
ItemEvents.rightClicked('tfcobblemon:blank_orb', event=> {
        if (event.player.mainHandItem.id == "tfcobblemon:blank_orb") {
            const orb = Item.of('tfcobblemon:life_orb_charging');
            orb.damageValue = 100;
            event.player.mainHandItem = orb;
            event.server.runCommandSilent(`playsound minecraft:entity.allay.ambient_with_item block ${event.player.username} ${event.player.x} ${event.player.y} ${event.player.z} 3`)
    }
})
...
ItemEvents.rightClicked('tfcobblemon:life_orb_charging', event=> {
  if (event.player.mainHandItem.id == "tfcobblemon:life_orb_charging") {
    if (event.player.mainHandItem.damageValue <= 0) {
      ...
      event.server.runCommandSilent(`playsound minecraft:entity.allay.ambient_without_item block ${event.player.username} ${event.player.x} ${event.player.y} ${event.player.z} 3`)
    }
}
})
    ...
    if (nearestPlayer !== null) {
        ...
        if (index !== -1) {  
            ...
            if (orb.damageValue >= 1) {
                orb.damageValue--;
                event.server.runCommandSilent(`playsound minecraft:block.amethyst_cluster.hit block ${nearestPlayer.username} ${nearestPlayer.x} ${nearestPlayer.y} ${nearestPlayer.z} 3`);
            }
        }
    }
});
