...
ItemEvents.rightClicked(event => {
    ...
    if (itemId.startsWith("tfcobblemon:golett_")) {
        ...
        event.server.runCommandSilent(`playsound tfc:item.ceramic.break block ${event.player.username} ${event.player.x} ${event.player.y} ${event.player.z} 3`)
        event.item.count--;
        event.server.runCommandSilent(`give ${event.player.username} tfcobblemon:blank_orb`)
    }
});
